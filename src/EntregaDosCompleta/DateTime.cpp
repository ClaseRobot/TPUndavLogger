...
string ULDateTime::ToFormat(const DateTime *dateTime, DateTimeFormat format)
{
    ...
    case DDMMYYYY_HHmmss:
        ...
        break;
    case YYYYMMDDHHmmss:
        resultado = to_string(dateTime->year) + ParsearDosDigitos(dateTime->month) + ParsearDosDigitos(dateTime->day) + ParsearDosDigitos(dateTime->hour) + ParsearDosDigitos(dateTime->minute) + ParsearDosDigitos(dateTime->seconds);
        break;
    case YYMD_Hms:
...
string PrimerParseoFechaHora(const DateTime *dateTime, bool formato)
{
    string tiempo = ParsearDosDigitos(dateTime->year) + "-" + ParsearDosDigitos(dateTime->month) + "-" + ParsearDosDigitos(dateTime->day) + " ";
...
string SegundoParseoFechaHora(const DateTime *dateTime, bool formato)
{
    string tiempo = ParsearDosDigitos(dateTime->day) + "-" + ParsearDosDigitos(dateTime->month) + "-" + ParsearDosDigitos(dateTime->year) + " ";
    if (formato && dateTime->hour > 12)
    {
        tiempo += ParsearDosDigitos(dateTime->hour - 12);
    }
